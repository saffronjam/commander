...
> **Architecture note (current).** The frontend is GraphQL-native: it talks to one same-origin
> `/graphql` endpoint via **urql** (HTTP queries/mutations) and **graphql-ws** (subscriptions),
> with types generated by **graphql-codegen** into `src/gql` (run `bun run codegen` after editing
> a document or the schema). Live data arrives through `src/contexts/api/ApiProvider.tsx`
> (~32 `useSubscription` hooks feeding `ApiData`); the per-domain mappers in
> `src/contexts/api/live*.ts` convert generated GraphQL types into the `apiTypes.ts` domain shapes
> the components consume. There is **no SSE, no REST `fetch`, no `config.ts`/runtime-config**, and
> auth rides a same-origin HTTP-only cookie. In production the SPA is embedded in and served by the
> Go binary on the same origin; in dev, Vite (`:3039`) proxies `/graphql` to the backend (`:8081`).
>
> Sections below that mention Server-Sent Events, `make generate`/tygo as the type source, or
> Redis are **historical** — follow the architecture note.

## Overview

React-based dashboard for monitoring Satisfactory factory operations. Features real-time data visualization with charts, interactive maps, and live updates via GraphQL subscriptions.
